...
function JoinForm(props) {
  // 자식 컴포넌트인 JoinForm 에서 state 로 설정한 변수
  ...
  const submitClick = e => {
    ...
    // 서버로 전달할 데이터가 없으므로 submit 이벤트를 중지(현재 이벤트 정지)
    e.preventDefault();
  }
  ...
  return (
    <form onSubmit={submitClick}>
      ...
      <div className={'mt-3 d-grid gap-2'}>
        ...
        <button type={'reset'} className={'btn btn-secondary'} onChange={resetClick}>취소</button>
      </div>
    </form>
  );
