...
const reservedNotifications = [
    {message:'안녕하세요. 메시지 테스트1 입니다.', key:1},
    {message:'안녕하세요. 메시지 테스트2 입니다.', key:2},
    {message:'안녕하세요. 메시지 테스트3 입니다.', key:3},
    {message:'안녕하세요. 메시지 테스트4 입니다.', key:4},
    {message:'안녕하세요. 메시지 테스트5 입니다.', key:5},
];
...
class NotificationList extends React.Component {
    ...
    constructor(props) {
        ...
        // state 객체 선언
        this.state = {
            // notications 라는 이름의 배열을 state 로 상태관리
            notifications: [],
            // key : notifications, value : 빈 배열
        };
    }
    ...
    componentDidMount() {
        ...
        timer = setInterval(() => {
            ...
            if (notifications.length < reservedNotifications.length) {
                ...
                // 위에서 지정한 index 값을 사용하여 전역 변수 reservedNotifications 배열의 데이터를 꺼내어서 지역 변수 notifications 배열에 추가 
        ...
        return (
            <div>
                ...
                {this.state.notifications.map((item) => {
                    // key : 자식 컴포넌트를 반복(map, forEach 등) 하여 호출할 경우 리액트는 호출한 자식 컴포넌트를 구부하지 못함
                    // 반복 호출한 컴포넌트의 순서를 지정하기 위해서 사용하는 속성이 key 임
                    // key 는 중복되지 않는 값을 입력해야 함
