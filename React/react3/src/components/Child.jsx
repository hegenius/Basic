...
function Child(props) {
    // 부모 컴포넌트에서 전달한 값을 자식 컴포넌트의 변수의 단순 대입
    ...
    const btn3Event = () => {
    // props 데이터는 부모 컴포넌트에서 전달받은 읽기 전용의 데이터인데 이를 수정하려고 하기 때문에 에러남
    ...
    // 현재 컴포넌트에서 직접 선언하고 state로 관리하고 있는 변수의 값을 setState()로 수정
    const btn5Event = () => {
        setData3(++data3);
    ...
    return(
        <div className={'border border-2 border-primary rounded-2 p-3'}>
            ...
            <button type={'button'} className={'btn btn-info me-2 mb-2'} onClick={btn1Event}>props.data1의 값을 받은 data1
                수정
            </button>
            <button type={'button'} className={'btn btn-info me-2 mb-2'} onClick={btn2Event}>props.data2의 값을 받은 data2
                수정
            </button>
            <br/>
            <button type={'button'} className={'btn btn-success me-2 mb-2'} onClick={btn3Event}>자식 컴포넌트가 props.data1 직접
                수정
            </button>
            <button type={'button'} className={'btn btn-success me-2 mb-2'} onClick={btn4Event}>자식 컴포넌트가 props.data2 직접
                수정
            </button>
            <br/>
            <button type={'button'} className={'btn btn-primary me-2 mb-2'} onClick={btn5Event}>자식 컴포넌트가 state 값 수정
            </button>
            <button type={'button'} className={'btn btn-primary mb-2'} onClick={btn6Event}>props.data2의 값을 자식 컴포넌트의
                state로 받고 setState로 수정
            </button>
        </div>
