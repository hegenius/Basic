...
class SetStateClass extends React.Component {
    ...
    // SetStateClass 클래스의 멤버 메소드 StteChange 선언
    StateChange = flag => {
        if (flag == 'direct') {
            // 컴포넌트의 상태 정보를 저장하는 state 의 요소 중 StateString의 값을 = 연산자를 사용하여 직접 변경
            // 리액트에서는 Virtual DOM 사용하기 때문에 state의 값을 직접 수정 시 Virtual DOM 에만 적용되고, 실제 DOM에는 적용되지 않음  
    ...
    render() {
        return (
            <div>
                ...
                {/* jsx 문법에서 기본적으로 모든 html 태그 및 html 태그의 속성을 제공하지만 용어가 겹치는 것들이 존재하기 때문에 몇몇 속성은 다른 이름을 사용하고 있음 */}
