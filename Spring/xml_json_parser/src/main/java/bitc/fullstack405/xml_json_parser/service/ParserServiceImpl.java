...
//  Marshaller(마샬) : 자바 클래스를 xml 데이터로 변환 
//  Unmarshaller(언마샬) : xml 데이터를 자바 클래스 타입의 객체로 변환
...
//    json 파싱 라이브러리에는 네트워크 데이터 처리 부분이 없기 때문에 InputStreamReader 를 통하여 네트워크에서 데이터를 가져오는 부분과 가져온 데이터를 처리하는 부분이 필요함
