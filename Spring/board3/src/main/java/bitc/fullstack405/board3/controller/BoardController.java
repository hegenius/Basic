...
//  RESTful 이란 : URL을 자원의 이름으로 구분하여 해당 자원의 정보를 주고 받는 형식
//  HTTP URI 를 통해 자원을 명시하고, HTTP Method(GET, POST, PUT, DELETE)를 통해 자원에 대한 CRUD 명령을 적용하는 방식
...
//  Spring Framework의 템플릿에서 GET, POST 방식만 인식하기 때문에 PUT, DELETE 를 인식하기 위해서 input type=hidden 을 추가하고, name=_method 속성을
//  사용하여 데이터 전송 방식(PUT, DELETE)을 지정해야 함
...
// URL 이 동일하더라도 method 타입이 다르면 동일한 URL 을 사용할 수 있음
...
// @PutMapping : 클라이언트에서 데이터 전송 방식을 PUT 으로 설정한 URL만 접속, @RequestMapping(method = RequestMethod.PUT) 와 동일한 방식
//  게시물 수정
...
//  @DeleteMapping : 클라이언트에서 데이터 전송 방식을 DELETE 으로 설정한 URL만 접속, @RequestMapping(method = RequestMethod.DELETE) 와 동일한 방식
