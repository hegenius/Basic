...
// @Controller 의 기능인 스프링 프레임워크에 해당 클래스가 Controller 임을 알려주고 클라이언트에서 접속하는 URL과 메소드를 연결하는 기능, URL과 메소드를 연결하는 기능과 @ResponseBody 의 기능인 클라이언트에게 View 없이 데이터만 전달하는 기능이 합쳐진 어노테이션
...
// RESTful API : REST 방식과 동일한 형태로 HTTP 프로토콜을 사용하지만, 클라이언트로 전달하는 데이터가 View 를 제외한 순수 데이터만 전달하는 방식
// 예전에는 클라이언트가 컴퓨터 밖에 없었기 때문에 웹 브라우저 혹은 전용 프로그램이 클라이언트의 기능을 모두 담당했음
// 서버의 기능의 웹서버의 기능만 주로 사용되었기 때문에 클라이언트로 전달하는 데이터가 View 와 Model 데이터 모두 필요한 형태였음
...
//  게시판 글 상세 정보 가져오기 API
  ...
  public Object selectBoard(@PathVariable("boardIdx") int boardIdx) throws Exception {
...
//  @RequestParam은 서버주소(URL)에 데이터를 포함하여 전달하는 방식
//  게시판 글 등록 API
