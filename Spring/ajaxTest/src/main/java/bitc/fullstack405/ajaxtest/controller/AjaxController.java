...
//  method : 사용자가 서버로 데이터 전달 시 사용한 통신 방식을 설정 (GET, POST), Restfull 방식 사용 시 GET, POST, PUT, DELETE 를 사용할 수 있음

//  @ResponseBody : 응답으로 리턴할 데이터를 html 템플릿이 아닌 데이터 자체를 전달하도록 하는 어노테이션, ajax 통신 시 반드시 필요함 (사용안하면 html 태그가 날아가서 오류남)
  @ResponseBody
...
//    result = areaService.calculatorResult(num1, num2, op);
  ...
  public Object quiz2Addr1(@RequestParam ("areaName") String areaName) throws Exception {
    //    서비스를 사용하여 구 정보를 가져옴
    List<AreaDTO> districList = areaService.getDistrictList(areaName);
