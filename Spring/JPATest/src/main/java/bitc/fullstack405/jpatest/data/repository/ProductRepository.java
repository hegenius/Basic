...
//  find ... By : select 명령을 수행하는 키워드, read ... By, get ... By, query ... By, search ... By, stream ... By 등이 추가로 존재함
  Optional<ProductEntity> findProductEntityByNumber(Long number); // 기본 사용 방식 0개의 사용위치
...
//  exists ... By : 특정 데이터가 존재하는지 여부를 확인하는 키워드, true/false 반환
  boolean existsByNumber(Long number); // 지정한 번호의 데이터가 있는 지 확인
...
//  (Is)Not : 값의 불일치 조건을 사용하는 조건 키워드, Is 생략할 수 있음
  ...
  ProductEntity findByNumberNot(Long number); // 지정한 번호와 같이 않은 데이터를 조회
  
//  (Is)Null, (Is)NotNull : 값이 null인지 확인하는 조건 키워드, Is 생략 가능
  List<ProductEntity> findByUpdatedAtNull(); // 수정된 데이터가 null인 것만 조회
  List<ProductEntity> findByUpdatedAtIsNull(); // 수정된 데이터가 null인 것만 조회
...
//  (Is)GreaterThan, (Is)LessThan, (Is)Between : 숫자나 datetime 컬럼을 대상으로 비교 연산에 사용할 수 있는 조건 키워드
...
//  EndingWith : 키워드가 끝 문자열일 경우
  List<ProductEntity> findByNameEndsWith(String name); // 지정한 문자열로 끝나는 이름조회
...
//  @Query("SELECT p FROM ProductEntity AS p WHERE p.name = ?1 AND p.price = ?2") // 이렇게도 사용 가능
