...
<%--    property : 지정한 객체가 가지고 있는 필드에 저장할 데이터 --%>
<%--    value : 지정한 객체가 가지고 있는 필드에 저장할 데이터 --%>
    <jsp:setProperty name="person" property="name" value="정재원"/>
    <jsp:setProperty name="person" property="age" value="35"/>

    <h3 class="text-center">getProperty 액션 태그 사용</h3>
    <ul class="list-group">
<%--    getProperty : useBean 액션태그를 통해서 생성된 자바 클래스 타입의 객체에 데이터를 출력하는 액션 태그 --%>
<%--    name : useBean 액션 태그로 생성된 자바 클래스 타입의 객체명을 지정 --%>
