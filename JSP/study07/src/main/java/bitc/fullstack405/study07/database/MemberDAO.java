...
// 데이터베이스 접속 정보 및 연결 메소드를 JDBConnect 클래스에서 생성
...
//            select 문의 결과가 무조건 1개는 존재하기 때문에 rs.next()는 언제나 true 가 됨
            if (rs.next()) {
//                가져온 결과가 1이면 로그인 가능, 0이면 같은 정보 없어서 로그인 불가능
                if (rs.getInt("cnt") == 1) {
                    result = true;
                }
            }
...
//    회원 정보 조회 메소드
    public MemberDTO getMemberInfo(String id) {
            ...
            try {
                ...
                System.out.println("데이터베이스에 조회 중 오류가 발생했습니다.");
                e.printStackTrace();
            }
            finally {
                try{
