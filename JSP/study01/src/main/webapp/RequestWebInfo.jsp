...
<%--  URL고 URI의 차이 : URL은 네트워크 상에서 자원이 어디 있는지 위치를 알려주기 위한 규약, URI 는 인터넷에 있는 자원을 어디에 있는지 자원 자체를 식별하는 방법 --%>
...
<%--  getRemoteAddr() : 현재 서버에 접속에 사용한 클라이언트의 IP 주소를 출력 --%>
    <li class="list-group-item">클라이언트 IP 주소 : <%= request.getRemoteAddr() %></li> <%-- IPv6 --%>
<%--  getQueryString() : URI 뒤에 있는 ? 기호와 함께 전달된 데이터를 의미, GET 방식 사용 시 URL과 함께 전달된 데이터를 출력, POST 방식 사용 시 쿼리스트링은 null 임 --%>
    <li class="list-group-item">쿼리스트링 : <%= request.getQueryString() %></li>  <%-- URLDecode, encoding --%>
