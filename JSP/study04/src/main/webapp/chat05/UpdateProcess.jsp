...
<body>
<%
    // 클라이언트에서 전달받은 데이터의 문자셋을 설정 UTF-8로 설정
...
<div class="container mt-5">
  <h2 class="text-center">Statement로 데이터 수정하기</h2>
    ...
    <%
        ...
        Statement stmt = null;
        // sql update 문 생성
        // 자바 소스 코드 내에서 sql 쿼리문을 직접 사용할 경우 sql 쿠리문의 문자열이 길기 때문에 += 연산자를 사용하여 여러줄의 sql 쿼리문을 연결해서 사용하게 됨
...
//            Statement 객체를 통해서 sql 문을 데이터베이스로 전달
//            executeUpdate() 로 실행
        ...
        finally { // 외부 리소스는 반드시 finally 를 사용하여 리소스 해제를 해야 함
            try {
                if (stmt != null) {
                    stmt.close();
                }
