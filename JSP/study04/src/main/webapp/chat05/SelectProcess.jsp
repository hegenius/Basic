...
<div class="container mt-5">
    ...
    <%
        ...
        // PrepareStatement : 복잡한 형태의 sql 쿼리문을 사용할 경우 사용, 여러번 재활용할 수 있음, sql 문 내에 ? 특수기호를 사용하고 나중에 해당 위치에 데이터를 추가하여  sql 쿼리문을 완성함
        Statement stmt = null;
        ...
        try {
            ...
            // getString() : ResultSet 클래스에서 제공하는 데이터 출력 메소드, String 타입으로 데이터를 출력, 매개변수로 컬럼명을 사용하여 데이터 출력 매개변수로 index 번호도 사용 가능 (컬럼명을 주로 사용)
            // getInt() : ResultSet 클래스에서 제공하는 데이터 출력 메소드, int 타입으로 데이터를 출력
            // getXXX() : ResultSet 클래스에서 제공하는 데이터 출력 메소드이며 모든 데이터 타입 메소드가 다 존재함 (getDouble : 소수점)
            while (rs.next()) {
            ...
            if (conn != null) {conn.close();} // 먼저 열림, 먼저 열린거 제일 마지막에 닫아야함
        }
    %>
</div>
</body>
