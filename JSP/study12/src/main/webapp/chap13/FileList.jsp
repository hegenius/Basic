...
<%-- 기본 스크립틀릿으로 출력--%>
<%--            <%--%>
<%--                // DB에서 가져온 내용을 view 파일인 FileList 출력--%>
...
<%--            JSTL 출력 --%>
            <c:forEach var="item" items="${fileList}">
                <tr>
                    ...
                    <td>${item.sfile}</td>
                    <td>${item.postDate}</td>
<%--                    업로드된 파일을 다운로드 하기 위해서 Download.jsp 로 다운로드 요청 --%>
<%--                    파라미터로 원본 파일명과 수정된 파일명을 전달함 --%>
<%--                    URLEncoder : URL로 영어를 전송 시 문제가 없지만 한글은 URL로 전송 시 문자 깨짐, URLEncoder를 사용하여 한글을 변환하여 전송 --%>
