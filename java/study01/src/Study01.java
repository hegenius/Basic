...
//        1. 변수명은 반드시 영어로 입력
//        2. 변수명에 영문자 및 숫자, 특수문자를($, _)를 사용 가능
//        3. 변수명의 첫글자는 반드시 영문자이여야 함
//        4. 영어 대소문자를 판별함
//        5. 2개 이상의 단어로 조합된 이름의 경우 첫번째 단어는 모두 영어 소문자로 입력, 두번째 이상의 단어는 첫 글자는 영어 대문자, 두번재 글자이후로는 영어 소문자로 입력
//        6. 자바 예약어는 변수명으로 사용할 수 없음

        int apple = 10;
        int 사과 = 100; // 한글로 변수명을 입력해도 오류가 발생하지 않음, 관례적으로 프로그램 시 데이터를 제외하고는 영어로 입력하는 것이 좋음
        System.out.println(apple);
        System.out.println(사과);

        int iphone15 = 150; // 변수 이름에 숫자 사용 가능
        int iphone15max = 180;
        int iphone15$ = 150; // 변수 이름에 특수문자 $, _ 사용 가능
//        int 15&*#@##@! = 150;  // 변수 이름에 $, _를 제외한 특수문자는 사용 불가
